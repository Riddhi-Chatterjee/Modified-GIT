...
def forward_backward_batch(image_files, captions, prefixs, cfg, model, tokenizer, episode, batch):
    all_data = []
    ...
    loss.backward()
    print("Episode: "+str(episode)+" Batch: "+str(batch)+" Loss: "+str(loss.item()))

def forward_backward_example(image_files, captions, prefixs=None, batch_size = 128, num_episodes = 100):
    if prefixs is None:
        prefixs = ['' for i in range(len(captions))]
    ...
    for episode in range(num_episodes):
        ...
        while(tot_files > 0):
            ...
            end = start + min(tot_files, batch_size)
            forward_backward_batch(image_files[start:end], captions[start:end], prefixs[start:end], cfg, model, tokenizer, episode+1, batch_num)
